...
| DNS/TLS             | App Service managed certificate plus Cloudflare record | Publish ownership proof first; approve and monitor a maximum ten-minute TLS interruption window, move the CNAME directly to the target, then issue/bind the managed certificate; restore the captured source record immediately on failure or deadline |
...
- [ ] Publish ownership proof, approve a monitored maximum ten-minute TLS interruption window, atomically move the CNAME to the target, then issue/bind TLS and run target acceptance; revert DNS immediately if issuance or acceptance fails or the deadline is reached.
