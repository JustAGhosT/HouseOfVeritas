...
| DNS/TLS             | App Service managed certificate plus Cloudflare record | Publish ownership proof first; approve and monitor a maximum ten-minute TLS interruption window; keep the HOV CNAME DNS-only with flattening disabled; after moving it, verify public DNS returns the target `azurewebsites.net` CNAME directly, then issue/bind the managed certificate; restore the captured source record immediately on failure or deadline |
...
- [ ] Publish ownership proof, approve a monitored maximum ten-minute TLS interruption window, set a DNS-only unflattened CNAME directly to the target, verify the public CNAME answer, then issue/bind the App Service managed certificate and run target acceptance; retain DNS-only/unflattened mode for renewal and revert DNS immediately if issuance or acceptance fails or the deadline is reached.
