# Network Module - VNet, Subnets, NSGs for House of Veritas
...
resource "azurerm_subnet" "gateway" {
  name                 = "${var.environment}-gateway-subnet"
  resource_group_name  = var.resource_group_name
...
resource "azurerm_network_security_group" "containers" {
  name                = "${var.environment}-container-nsg"
  location            = var.location
  resource_group_name = var.resource_group_name

  security_rule {
    name                       = "AllowGatewayDocuSeal"
    priority                   = 100
    direction                  = "Inbound"
    access                     = "Allow"
    protocol                   = "Tcp"
...
resource "azurerm_network_security_group" "database" {
  ...
  security_rule {
    ...
    source_address_prefix      = var.container_subnet_prefix
    destination_address_prefix = "*"
  }
