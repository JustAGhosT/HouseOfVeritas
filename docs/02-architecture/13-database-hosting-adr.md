...
4. **The server is codified but not yet adopted.** It was created with `az` during the migration. It is now described in an org-level stack — [`neuralliquid-org/infra/terraform/shared-data`](https://github.com/neuralliquid/neuralliquid-org/tree/main/infra/terraform/shared-data), governed by that repo's [ADR 0002](https://github.com/neuralliquid/neuralliquid-org/blob/main/docs/adr/0002-shared-data-plane-ownership.md) — with import blocks whose plan reads *7 to import, 1 to change, 0 to destroy*. Until that plan is applied it remains drift; the single change is adding the `environment` and `project` tags the server was created without.
...
- `DATABASE_URL` for HOV points at `nl-prod-shared-pg.postgres.database.azure.com/houseofveritas`, stored at `nl-prod-convolens-kv/hov-estate-database-url`. That vault is the wrong home twice over: it holds HOV's connection string, which belongs in `nl-prod-hov-kv`, and the shared server's admin password (`shared-pg-admin-password`), which belongs in an org-owned vault in `nl-prod-shared-rg`. Convolens' vault should end up holding only convolens' own secrets.
