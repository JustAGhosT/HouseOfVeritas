...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `/api/files`   | Azure Blob (`asset-photos` category → `asset-photos`; `invoices` → `invoice-scans`; documents/other categories → configured `AZURE_STORAGE_CONTAINER`) or `/tmp/uploads` | PostgreSQL-authoritative opaque file mapping and owner hash, with Blob tags/properties as secondary evidence; local fallback returned only |
