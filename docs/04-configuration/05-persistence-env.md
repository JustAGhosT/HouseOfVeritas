...
The `house_of_veritas` database is created automatically when using Docker Compose (see `config/docker-compose.yml`). The NexaMesh migration stack provisions the production `houseofveritas` database on HOV's dedicated PostgreSQL server. Bootstrap separate schema-owner and application runtime roles after the runtime identity exists. The runtime role must be a member of the approved schema-owner role required by the current application DDL model, must not hold `SUPERUSER`, `CREATEDB`, `CREATEROLE`, or `REPLICATION`, and must be denied access to other product databases. Revoke `CONNECT` from `PUBLIC` on the HOV database and verify the positive and negative controls with `scripts/migration/hov-nexamesh/Test-PostgresControls.ps1`. Migration execution and source retirement remain separately approval-gated.
